...
extension WeatherViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            DispatchQueue.main.async {
                cell.configure(model: self.dailyWeather[indexPath.row], textForDay: indexPath.row == 0 ? "Today".localizable() : self.dailyWeather[indexPath.row].dateTime.timeIntervalToStringDate(.shortDate))
            }
            return cell
        }
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
            ...
            label.text = section == 0 ? "HOURLY FORECAST".localizable() : "8-DAY FORECAST".localizable()
            return header
    }
}
...
extension WeatherViewController: UITextFieldDelegate {
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        let mySet = CharacterSet(charactersIn: "-")
        let newSet = mySet.union(CharacterSet.whitespaces)
        guard CharacterSet.alphanumerics.union(newSet).isSuperset(of: CharacterSet(charactersIn: string))  else {
            return false
        }
        return true
    }
